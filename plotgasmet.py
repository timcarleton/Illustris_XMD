import getgasinfo2_40_100 as getgasinfo2
import imp
...
print(len(res1[0]),len(res2[0]),len(res3[0]),len(res4[0]))
f=fits.open('alltnggal_withr_wmetals_wvmax.fits')
obj=ascii.read('sampleselection_sfr5_notide_nomass.txt')
...
plt.savefig('gasevolution_100.png')
...
plt.yscale('log')
plt.xscale('log')
plt.savefig('gasevolution_dist1_100.png')
...
plt.yscale('log')
plt.xscale('log')
plt.savefig('gasevolution_dist_100.png')
...
plt.legend()
plt.yscale('log')
plt.savefig('gasdistevolution_100.png')
...
plt.legend()
plt.yscale('log')
plt.savefig('gasevolution_distsfr_100.png')
...
plt.ylim(1E-5,1E-2)
plt.yscale('log')
plt.savefig('gasevolution_timesfr_100.png')
...
plt.ylim(1E-5,2E-2)
plt.yscale('log')
plt.savefig('gasevolution_timesfr_100_cum.png')
...
plt.plot(np.nansum(avgsfr4*np.tile(deltat,len(wlmda)).reshape(len(wlmda),20),axis=1)*1E9,f[1].data.gas_metal_sfr[obj['allgalindex'][wlmda]],'C3h')
plt.plot(np.nansum(avgsfr3*np.tile(deltat,len(wxmda)).reshape(len(wxmda),20),axis=1)*1E9,f[1].data.gas_metal_sfr[obj['allgalindex'][wxmda]],'C2s')
plt.plot(np.nansum(avgsfr2*np.tile(deltat,len(wlmd)).reshape(len(wlmd),20),axis=1)*1E9,f[1].data.gas_metal_sfr[obj['allgalindex'][wlmd]],'C1x')
plt.plot(np.nansum(avgsfr1*np.tile(deltat,len(wxmd)).reshape(len(wxmd),20),axis=1)*1E9,f[1].data.gas_metal_sfr[obj['allgalindex'][wxmd]],'C0*')
plt.loglog()
...
cumsfrall=np.hstack([np.nansum(avgsfr4*np.tile(deltat,len(wlmda)).reshape(len(wlmda),20),axis=1)*1E9,np.nansum(avgsfr3*np.tile(deltat,len(wxmda)).reshape(len(wxmda),20),axis=1)*1E9,np.nansum(avgsfr2*np.tile(deltat,len(wlmd)).reshape(len(wlmd),20),axis=1)*1E9,np.nansum(avgsfr1*np.tile(deltat,len(wxmd)).reshape(len(wxmd),20),axis=1)*1E9])
