...
plt.savefig('corretimescale.png')
...
plt.savefig('corretimescaleall.png')

from astropy.io import ascii
x=ascii.read('mstagesizeinfo_ca.txt')
wxmd=np.where(x['type']==1)[0]
wxmda=np.where(x['type']==3)[0]
print(np.mean(x['myoung'][wxmd]/x['mall'][wxmd]))
print(np.mean(x['myoung'][wxmda]/x['mall'][wxmda]))

print(np.mean((x['mmid'][wxmd]+x['mmid2'][wxmd])/x['mall'][wxmd]))
print(np.mean((x['mmid'][wxmda]+x['mmid2'][wxmda])/x['mall'][wxmda]))

print(np.mean(x['mold'][wxmd]/x['mall'][wxmd]))
print(np.mean(x['mold'][wxmda]/x['mall'][wxmda]))

print(np.nanmedian(x['ryoung'][wxmd]-x['rall'][wxmd]))
print(np.nanmedian(x['ryoung'][wxmda]-x['rall'][wxmda]))
