...
def getoutflow(typei,redo=False):
    wcat=np.where(obj[:,2]==typei)[0]
    gflows=glob.glob('flowinfo/flowinfo_100*txt')
    #for i in range(10):
    #    for j in range(99):
    ...
    for i in wcat:
        ...
        if 'flowinfo/flowinfo_100_'+str(typei)+'_'+str(int(obj[i,1]))+'.txt' in gflows and not redo:
            continue
        
        print(i,obj[i,1])
        g=glob.glob('/Volumes/Elements/illustris/cutouts/'+str(int(obj[i,1]))+'/*_*hdf5')
        if len(g)==0:
            print('no files')
            continue
        ...
        for j in ssnaps:
            ...
            print('/Volumes/Elements/illustris/cutouts/'+str(int(obj[i,1]))+'/'+str(int(obj[i,1]))+'_'+str(j)+'.hdf5')
            h=h5py.File('/Volumes/Elements/illustris/cutouts/'+str(int(obj[i,1]))+'/'+str(int(obj[i,1]))+'_'+str(j)+'.hdf5','r')
        ...
        tab.write('flowinfo/flowinfo_100_'+str(typei)+'_'+str(int(obj[i,1]))+'.txt',format='csv',overwrite=True)
