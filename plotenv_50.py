...
for i in range(len(x)):
    ...
    dist=getperiodicdist.getperiodicdist(np.array([f[1].data.x[x['allgalindex'][i]],f[1].data.y[x['allgalindex'][i]],f[1].data.z[x['allgalindex'][i]]]),np.array([f[1].data.x[wmst],f[1].data.y[wmst],f[1].data.z[wmst]]).T,boxsize=35/.6774)
    s=np.argsort(dist)
    d5.append(dist[s[6]])
...
whigh=np.where(f[1].data.gas_metal_sfr[x['allgalindex'][wlmda]]>1.25)[0]
print(len(whigh),len(np.where(d5[wlmda[whigh]]<1000)[0]))

import bindata
plt.clf()
#plt.plot(d5,f[1].data.gas_metal_sfr[x['allgalindex']],'o',alpha=.2)
plt.plot(d5[wlmda],f[1].data.gas_metal_sfr[x['allgalindex'][wlmda]],'C3h',label='LMD Analogs')
plt.plot(d5[wxmda],f[1].data.gas_metal_sfr[x['allgalindex'][wxmda]],'C2s',label='XMD Analogs')
plt.plot(d5[wlmd],f[1].data.gas_metal_sfr[x['allgalindex'][wlmd]],'C1x',label='LMDs')
plt.plot(d5[wxmd],f[1].data.gas_metal_sfr[x['allgalindex'][wxmd]],'C0*',label='XMDs')
plt.yscale('log')
b=bindata.bindata(d5,np.log10(f[1].data.gas_metal_sfr[x['allgalindex']]),np.linspace(0,5000,num=40))
plt.plot(b[2],10**b[0],'k',label='Median Trend for all Galaxies')
#plt.xscale('log')
print(stats.spearmanr(d5,f[1].data.gas_metal_sfr[x['allgalindex']]))
wv=np.where(d5>1000)[0]

print(len(wv),len(d5))
print(stats.spearmanr(d5[wv],f[1].data.gas_metal_sfr[x['allgalindex'][wv]]))
plt.ylabel(r'$Z_{\rm SFR}/Z_\odot$')
plt.xlabel(r'$d_5$ (kpc)')
plt.legend()
plt.gca().xaxis.set_minor_locator(ticker.MultipleLocator(100))  
plt.title('TNG50',y=0.92,x=0.3)
plt.savefig('metvsd5_50.png')
...
plt.savefig('d5env_50.png',dpi=300)
...
plt.yscale('log')
plt.savefig('metvsdens5_50.png')

np.savetxt('tng50_d5.txt',np.array([x['allgalindex'],x['haloid'],x['type'],d5]).T)
